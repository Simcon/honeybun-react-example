# Honeybun React Example

This is an example React application with Honeybun OIDC integration.
...
Switch into the `example` folder and install the example's packages:

`npm install`

Set authority, client_id and redirect_uri in `index.tsx` to match your Honeybun application settings:

```
const oidcConfig = {
    authority: "https://auth.honeybun.io/",
    client_id: "YOUR APPLICATION'S CLIENT ID",
    redirect_uri: "YOUR REDIRECT URL (EG. http://localhost:1234 FOR TESTING)",
};
```

Start the example app and try logging in:
